...
if [ ! -d klee-out-2 ] ; then
    $KLEE_DIR/bin/klee $EXTRA_OPTIONS --precision --loop-breaking -max-time=1 -default-trip-count=5 -output-dir=klee-out-2 $BITCODE
    rm -f klee-last
fi
if [ ! -e out-2.txt ] ; then
...
if [ ! -d klee-out-3 ] ; then
    $KLEE_DIR/bin/klee $EXTRA_OPTIONS --precision --loop-breaking -max-time=1 -default-trip-count=10 -output-dir=klee-out-3 $BITCODE
    rm -f klee-last
fi
if [ ! -e out-3.txt ] ; then
...
if [ ! -d klee-out-4 ] ; then
    $KLEE_DIR/bin/klee $EXTRA_OPTIONS --precision --loop-breaking -max-time=1 -default-trip-count=15 -output-dir=klee-out-4 $BITCODE
    rm -f klee-last
fi
if [ ! -e out-4.txt ] ; then
