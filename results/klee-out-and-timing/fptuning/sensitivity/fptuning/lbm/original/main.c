...
/*
 * Compile with: gcc lbm.c main.c -o lbm -lm
 * Run with: ./lbm_ref 20 reference.dat 0 1 100_100_130_cf_a.of
 */
...
int main( int nArgs, char* arg[] ) {
	...
	__asm__ __volatile__("rdtscp; "         // serializing read of tsc
                     ...
                     : "=a"(startTSC)        // output to tsc variable
                     :
                     : "%rcx", "%rdx"); // rcx and rdx are clobbered
//printf("before initialize main");	
...
SimRoiStart();
	// printf("Passes ROI function");
	for( t = 1; t <= param.nTimeSteps; t++ ) {
		//printf("enters the loop to execute handleinoutflow");
...
//	ptlcall_switch_to_native();
	...
	__asm__ __volatile__("rdtscp; "         // serializing read of tsc
                     "shl $32,%%rdx; "  // shift higher 32 bits stored in rdx up
                     "or %%rdx,%%rax"   // and or onto rax
                     : "=a"(endTSC)        // output to tsc variable
                     :
                     : "%rcx", "%rdx"); // rcx and rdx are clobbered
//	printf("Cycles: %ld\n", endTSC - startTSC);
...
//	printf("enters Initialize");
  ...
  //      printf("Before call works");
	LBM_showGridStatistics( *srcGrid );
       // printf("call to showGridstats works");
...
//	printf("Enters finalize");
        LBM_showGridStatistics( *srcGrid );
  //      printf("completes finalize");
